...
PHP tradicionalmente captura `$this` en esta closure aunque nunca la useis. Eso tiene dos consecuencias:
...
- **Memoria retenida**: La closure arrastre consigo todo el objeto —sus propiedades, sus dependencias— cuando en realidad solo necesita… nada.
...
Los objetos que antes mantenían ciclos de referencia innecesarios now se liberan antes, lo que puede ejecutar destructores en un momento distinto. Esto es técnicamente BC pero es el comportamiento correcto y esperado.
...
El caching de closures stateless es casi obvio con retrospectiva — si la closure no tiene estado, ¿por qué instanciarla 10.000 veces? — pero la implementación requiere cuidado porque cambia la identidad del objeto. Que hayan documentado los tres cambios BC y los hayan discutido openly da confianza.
